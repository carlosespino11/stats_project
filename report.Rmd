---
title: "Understanding and Predicting Crime Rates Using Statistical Methods"
...
ggplot(Crime) + geom_bar(aes(x =factor(year), fill=crmrte_cat )) +
  scale_fill_fivethirtyeight() + theme_fivethirtyeight() + labs(title='Crime Rate by Year', x = "Year")
...
crime_region = ggplot(Crime) + geom_bar(aes(x = crmrte_cat, fill = crmrte_cat)) + facet_grid(region~.)+
  theme_fivethirtyeight()+ scale_fill_fivethirtyeight()  + labs(title = "Crime Rate by Region", x = "Crime Rate")
#candidate to test anova region west

crime_smsa = ggplot(Crime) + geom_bar(aes(x = crmrte_cat, fill = crmrte_cat)) + facet_grid(smsa~.)+
  theme_fivethirtyeight()+ scale_fill_fivethirtyeight()  + labs(title = "Crime Rate by SMSA", x = "Crime Rate")
...
lmPl1 = diagPlot(lm.fit) 
lmPl1$cdPlot
lev.p <- sort(cooks.distance(lm.fit)[cooks.distance(lm.fit)> (4/(nrow(Crime)-ncol(Crime))+1)/2 ])
```
...
```{r, echo=FALSE}
kable(data.frame(Cooks.Dist=lev.p))

Crime = Crime[-as.numeric(names(lev.p)),]
```
...
cCrime = Crime[-which(names(Crime) %in% c("crmrte_cat"))]
xs_test  = xs[test,]
xs_train = xs[-test,]
ys_test  = ys[test,]
ys_train = ys[-test,]
...
The cross-validation estimate of the training error is $`r min_train_error.1`$ and the cross-validation error is $`r min_test_error.1`$. The actual training and test errors for this subset, on the original datasets, are $`r train.errors.bestsubset`$ and $`r test.errors.bestsubset`$, respectively. Both were obtained when using _best subset_ with $k = 8$ predictors. The ratio between _testing_ and _training_ errors is ($`r test.errors.bestsubset / train.errors.bestsubset`$). Consequently, we can conclude that the predictors yielded by the subset generated using _best subset selection_ belong to a consistent model and, hence, can be used as a basis for non linear models. Nevertheless, we decided to run a linear fit with these predictors in order to check our conclusions: 
...
fitnames = c("prbarr" , "prbconv" , "polpc" , "density" ,
             ...
             "pctymle", "crmrte")
ggpairs(Crime[names(Crime) %in% fitnames])
```

It can be seen that the relationship between `crmrte` and `prbrarr`, `prbconv` and `polpc`, respectively, could be better explained by applying a _log_ to these predictors. Additionally, `wfed` and `pctmin` seem to have a nonlinear relationship with the response, which makes them suitable as polynomial regression predictors. Consequently, we run a new, nonlinear model with these modified predictors: 

```{R}
lm.3.fit = lm(crmrte ~  log(prbarr) + log(prbconv) +
                log(polpc) + density + as.factor(region) + 
                poly(pctmin,4) + poly(wfed,3) + pctymle, data = Crime)
summary(lm.3.fit)
```
...
```{R}
lm.4.fit = lm(crmrte ~  .*., data = cCrime)
```
...
lm.5.fit = lm(crmrte ~  log(prbarr) + log(prbconv) +
                log(polpc) + density + as.factor(region) + 
                poly(wfed,3) + pctymle + .*., data = cCrime)
```
...
```{R}
pred = predict(interaction.fit, cCrime[-test,])
...
The lack of significance of the polynomials for `wfed` and the increase in $R^2$ suggests that this model could possibly overfit. Hence, we removed the polynomials related to `wfed`, but kept the $log$ predictors as they have shown to be very significant. `pctmin` has a special behaviour, where we see that only the $3rd$ degree polynomial is used and seems less significant that the linear approach. Consequently, we decided to remove both the polynomial and the linear coefficients and re attach `pctmin` as a spline in \nameref{sec:nonlinear}. This updated model yielded a _testing MSE_ of $`r mean((ys_test - xs_predict)^2)`$, and an $R^2$ value of $`r summary(lm.3.fit)$r.squared`$.

Next, we analysed all possible interactions between all original predictors and plugged them to our previous model, yielding two new models. The number of interactions that we added to them is $`r dim(summary(lm.4.fit)$coefficients)[1]`$ and the $R^2$ values for each fit are $`r summary(lm.4.fit)$r.squared`$ and $`r summary(lm.5.fit)$r.squared`$, respectively. Consequently, we can affirm that both models are seriously overfitting because the number of predictors has skyrocketed due to all interaction combinations. Even though it is tempting to keep only those interactions with a relevant significance value, since the removal of each of these predictors affects the overall model, we chose instead to refine it by using a _Stepwise Algorithm_ applying _AIC_ to decide. The resulting fit has $`r length(interaction.fit$coefficients)[1]`$ coefficients, which means a reduction on the number of predictors by $`r ((dim(summary(lm.4.fit)$coefficients)[1] - length(interaction.fit$coefficients)[1]) / dim(summary(lm.4.fit)$coefficients)[1]) * 100 `\%$. We then proceeded to calculate both _training MSE_ ($`r val.train.errors`$) and _testing MSE_ ($`r val.test.errors`$). Now that we obtained a complex model consisting of linear variables, _log_ variables and _interaction_ variables, we will perform _Lasso_ in order to remove all interaction terms that are not significant, so that we arrive to a model easy to understand.
...
Lasso yielded the following `r length(lasso_vars)` non-zero predictors:

```{R}
kable(split(lasso_vars, ceiling(length(lasso_vars) / 5)))
```

```{R}
ys_lasso_pred = predict(lasso.mod.3, xs_lasso_test, s = lasso.mod$lambda.1se)
ys_lasso_pred_train = predict(lasso.mod.3, xs_lasso_train, s = lasso.mod$lambda.1se)
lasso_rss = mean((ys_test - ys_lasso_pred)^2)
lasso_train = mean((ys_train - ys_lasso_pred_train)^2)
```

```{R}
ys_lasso_modified = lm (crmrte ~  log(prbconv) + log(polpc) + poly(wfed,3) + prbarr + wfed + wsta + pctymle + county + region + density + pctmin:county + polpc:region + density:region + density:pctmin + density:pctymle + pctmin:region + region:wsta + pctymle:region + wsta + pctymle + county + region + density, data = cCrime[-test,])
ys_lasso_modified_pred_train = predict(ys_lasso_modified, cCrime[-test,])
ys_lasso_modified_pred_test = predict(ys_lasso_modified, cCrime[test,])
lasso_modified_test_mse = mean((ys_test - ys_lasso_modified_pred_test)^2)
lasso_modified_train_mse = mean((ys_train - ys_lasso_modified_pred_train)^2)
```

This means a reduction of `r round(((length(interaction.fit$coefficients) - length(lasso_vars)) / length(interaction.fit$coefficients))*100)`$\%$ with respect to the number of predictors returned by _stepwise AIC_. However, some of the interaction terms that appear involve predictors that were not in the original model (`wsta`, `pctymle`,   `county`, `region`, `pctmin` and `density`). Hence, we added these predictors (except `pctmin` which, as mentioned beforme, will be added as a spline) to the model in order to provide the final predictor set for this section. The _testing MSE_ for this model is $`r lasso_modified_test_mse`$ and this error is `r (lasso_modified_test_mse / test.errors.bestsubset) * 100`% of the _testing MSE_ of our original model using _best subset selection_.  Finally, we analysed the $L1$-norm between our estimated responses and the true model, also called _approximation error_, as:

$$ \frac{1}{n} \sum\limits_{i = 1}^{n}\Bigg|\frac{y_i - \hat{y}_{lasso_i}}{y_i}\Bigg|$$

which yielded a value of `r mean(abs((ys_test - ys_lasso_modified_pred_test)/ ys_test))`, which is more than reasonable since it's below $30\%$. Consequently, the final set obtained in this section, after performing _linear_, _best subset selection_, _log_, _polynomial_, _stepwise AIC_ and _Lasso_ yielded a model that will be used in the following sections for more complex fits that will derive in our final model.

## Non-linear Modeling
\label{sec:nonlinear}
From both \nameref{sec:lin_analysis} and the pairs analysis in \nameref{sec:dataset}, we identified a predictor `pctmin` showin  that could benefit from a more flexible modeling using poylnomial regression and splines and improve the overall prediction.
...
mse.lin = mean((pred.lin-Crime.test$crmrte)^2)
...
This naive model results in a MSE of $`r round(mse.lin,7)`$ and a mean approximation error rate of `r round(error.lin,4)` for the testing set. From the plot, it is clear that the relationship between the crime rate and the proportion of minorities in the area is not linear. 
...
mse.poly = mean((pred.poly-Crime.test$crmrte)^2)
...
This fit resulted in a MSE of $`r round(mse.poly,7)`$ and a mean approximation error rate of `r round(error.poly,4)`. The model was improved by reducing the bias of the model.

### Splines
```{r}
# Natural spline knots
pctmin.knots = attr(ns(Crime.train$pctmin, df=8), "knots")
...
Consequently, we see a modest improvement in the MSE ($`r round(mse.spl,7)`$) and the mean approximation error (`r round(error.spl,4)`).
...
for (i in 1:length(folds)) {
  fit.lin2[[i]] = lm(crmrte~pctmin + log(prbconv) + log(polpc) + poly(wfed,3) + prbarr + wfed + wsta + pctymle + county + region + density + pctmin:county + polpc:region + density:region + density:pctmin + density:pctymle + pctmin:region + region:wsta + pctymle:region, data=Crime.train[-folds[[i]],])
...
mse.lin2 = mean((pred.lin2-Crime.test$crmrte)^2)
error.lin2 = mean(abs((pred.lin2-Crime.test$crmrte)/Crime.test$crmrte))
```

Combining the predictors from the previous section with just the linear `pctmin` results in a MSE of $`r round(mse.lin2,8)`$ and a mean approximation error of `r round(error.lin2,4)`.
...
for (i in 1:length(folds)) {
  fit.spl2[[i]] = lm(crmrte ~ ns(pctmin, knots=pctmin.knots) + log(prbconv) + log(polpc) + poly(wfed,3) + wsta + pctymle + county + region + density + prbarr + wfed + pctmin:county + polpc:region + density:region + density:pctmin + density:pctymle + pctmin:region + region:wsta + pctymle:region, data=Crime.train[-folds[[i]],])
...
mse.spl2 = mean((pred.spl2$fit-Crime.test$crmrte)^2)
error.spl2 = mean(abs((pred.spl2$fit-Crime.test$crmrte)/Crime.test$crmrte))
```

Finally, we included the splined version of `pctmin` predictor alongside the selected predictors, which resulted in a slight improvement in the MSE to $`r round(mse.spl2,8)`$ and the mean approximation error to `r round(error.spl2, 4)`.

## Residual Analysis
To perform the residual analysis on our final project, we produced the following plot:

```{r}
lmPl = diagPlot(fit.spl2[[best.index.spl2]]) 
grid.arrange(lmPl$rvfPlot, lmPl$qqPlot, lmPl$sclLocPlot, lmPl$rvlevPlot, ncol=2)
```

We observe the following: 

* First, we notice that the residual variance is homoscedastic from the _Residual vs Fitted_  and the _Scale-Location_ plots , this means that it is constant. However the residuals are slighlty lower with smaller and larger values of the response variable. Additionally, you can see that the mean of the residuals is 0.

* The _Normal Q-Q Plot_ shows that the quantiles of the standardized residuals versus the quantiles of a standard normal are similiar, and meet the line reference almost perfectly except for the extreme points. This indicates that our residuals follow the normality hypothesis.

* The _Residual vs Leverage Plot_ exhibits that one of the residuals is highly leveraged and has a large Cook's distance, this corresponds to an observation which is both influential and an outlier. The rest of the residuals have a more regular behaviour.
...
```{r}
model = c("linear+best subset", "linear+interactions (LASSO)", "lasso results + splines")
training_mse = c(train.errors.bestsubset,
                 lasso_modified_train_mse,
                 mean((predict(fit.spl2[[best.index.spl2]], newdata=Crime.train, se.fit=TRUE)$fit-Crime.train$crmrte)^2))
testing_mse = c(test.errors.bestsubset, lasso_modified_test_mse, mse.spl2)
kable(data.frame(model = model, trainingMSE = training_mse, testingMSE = testing_mse))
```
