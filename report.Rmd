...
ggplot(Crime) + geom_bar(aes(x =factor(year), fill=crmrte_cat )) +
  scale_fill_fivethirtyeight() + theme_fivethirtyeight() + labs(title='Crime Rate by Year', x = "Crime Rate")
...
with nonzero values, a reduction of `r round(((length(interaction.fit$coefficients) - length(lasso_vars)) / length(interaction.fit$coefficients))*100)`$\%$ with respect to the _stepwise AIC_. The _test error_ for this model is obtained using the same _k-folds_ generated for the first model:  
...
for (i in 1:length(folds)) {
  fit.lin2[[i]] = lm(crmrte~pctmin + log(prbconv) + log(polpc) + poly(wfed, 3) + year + prbarr + wfed + pctmin:county + prbarr:wfir + polpc:region + density:region + density:pctmin + density:pctymle + pctmin:region + region:wsta + region:mix + pctymle:region, data=Crime.train[-folds[[i]],])
...
for (i in 1:length(folds)) {
  fit.spl2[[i]] = lm(crmrte ~ ns(pctmin, knots=pctmin.knots) + log(prbconv) + log(polpc) + poly(wfed, 3) + year + prbarr + wfed + pctmin:county + prbarr:wfir + polpc:region + density:region + density:pctmin + density:pctymle + pctmin:region + region:wsta + region:mix + pctymle:region, data=Crime.train[-folds[[i]],])
...
* First, we appreciate from the _Residuual vs Fitted_  and the _Scale-Location_ plots that the residual variance seems to be homoscedastic, this means that it is constant. However the residuals are slighlty lower with smaller and larger values of the response variable.

* The _Normal QQ-Plot_ shows that the quantiles of the standardized residuals versus the quantiles of a standard normal are similiar, and meet the line reference almost perfectly except for the extreme points. This indicates that our residuals follow the normality hypothesis.

* The _Residuual vs Leverage_ plot exhibits that we do not have high-leveraged residuals and that the Cook's distances are not large in general, indicating that we do not count with influential observations or outliers.
