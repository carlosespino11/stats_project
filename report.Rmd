...
for (i in 1:length(folds)) {
  fit.lin2[[i]] = lm(crmrte~pctmin + log(prbconv) + log(polpc) + poly(wfed,3) + prbarr + wfed + pctmin:county + polpc:region + density:region + density:pctmin + density:pctymle + pctmin:region + region:wsta + pctymle:region, data=Crime.train[-folds[[i]],])
...
for (i in 1:length(folds)) {
  fit.spl2[[i]] = lm(crmrte ~ ns(pctmin, knots=pctmin.knots) + log(prbconv) + log(polpc) + poly(wfed,3) + prbarr + wfed + pctmin:county + polpc:region + density:region + density:pctmin + density:pctymle + pctmin:region + region:wsta + pctymle:region, data=Crime.train[-folds[[i]],])
...
```{r}
model = c("linear+best subset", "linear+interactions (stepwise selection)", "linear+interactions (LASSO)", "lasso results + splines")
training_mse = c(train.errors.bestsubset, val.train.errors,
                 mean((ys_test -  predict(lasso.mod.3, xs_lasso_test, s = lasso.mod$lambda.1se))^2),
                 mean((predict(fit.spl2[[best.index.spl2]], newdata=Crime.train, se.fit=TRUE)$fit-Crime.train$crmrte)^2))
testing_mse = c(test.errors.bestsubset, val.test.errors, lasso_rss, mse.spl2)
